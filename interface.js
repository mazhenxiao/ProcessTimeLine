...
const option={
  ...
  class CreateDoc{
    ...
    async readFile(src){ //读取文件
        ...
        let clear = /(?:\/\*\*)|(?:\*\/)|\*\s*/ig;
        let pdfarr = /(?:@name|@param)\s.*|@template\s*[\s\S]+/ig;
        let reg = /\/\*\*[\s]+.*?(@name[\s\S]*@template[\s\S]*)\*\//ig;
        let name = /\*[\s]+@name[\s]+\[(.*?)\]/ig;
        let param =/\*[\s]+@param[\s](.+)/ig;
        let template =/\*[\s]+@template([\s\S]+)/ig;
        ...
        if(reg.test(str)){
          let md="";
          let html = md = str.match(reg)[0];
          let span = (str.match(param)||[]).map(da=>{
